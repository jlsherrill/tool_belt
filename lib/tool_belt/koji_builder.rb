...
module ToolBelt
  class KojiBuilder
    ...
    CLIENT_OSES = %w(rhel5 rhel6 rhel7 fedora20 fedora21)
    ...
    EXTERNAL_REPOS = {
      ...
      'fedora20' => ['fedora-20', 'fedora-20-updates'],
      'fedora21' => ['fedora-21', 'fedora-21-updates']
    }
    ...
    def build_katello_tags(os)
      ...
      operating_systems.each do |os|
        ...
        execute("add-external-repo rhscl-1.1-rhel-#{os.split('rhel')[1]} -t katello-#{@katello_version}-#{os}-build")
        execute("add-external-repo jpackage-5.0-generic-free -t katello-#{@katello_version}-#{os}-build")
